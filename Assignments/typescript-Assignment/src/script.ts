// ------------------------------------------------------------------
// Type Alias — restricts category to exactly these three strings
// ------------------------------------------------------------------
type Category = "Electronics" | "Clothing" | "Accessories";

// ------------------------------------------------------------------
// Interface — shape every product object must follow
// ------------------------------------------------------------------
interface Product {
  id: number | string;       // Union Type — ID can be a number OR a string
  name: string;
  price: number;
  category: Category;        // uses the Category type alias
  available: boolean;
  glyph: string;              // simple emoji stand-in for a product photo
}
...
function renderProducts(list: Product[]): void {
  ...
  const buttons = grid.querySelectorAll<HTMLButtonElement>(".view-btn");
